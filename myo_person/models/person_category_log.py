...
class PersonCategory(models.Model):
    ...
    log_ids = fields.One2many('myo.person.category.log', 'person_category_id', 'Person Category Log',
                              readonly=True)
    ...
    def insert_myo_person_category_log(self, person_category_id, values, action, notes):
        ...
        if self.active_log or active_log:
            ...
            self.pool.get('myo.person.category.log').create(self._cr, self._uid, vals)
    ...
    def write(self, values):
        ...
        return super(PersonCategory, self).write(values)

    @api.model
    def create(self, values):
        action = 'create'
        notes = False
        record = super(PersonCategory, self).create(values)
        record.insert_myo_person_category_log(record.id, values, action, notes)
        return record
