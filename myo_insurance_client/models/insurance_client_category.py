...
class InsuranceClientCategory(models.Model):
    _name = 'myo.insurance.client.category'

    name = fields.Char('Category', required=True, translate=True)
    parent_id = fields.Many2one('myo.insurance.client.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('myo.insurance.client.category', 'parent_id', 'Child Categories')
    ...
    insurance_client_ids = fields.Many2many(
        ...
        'myo_insurance_client_category_rel',
        'category_id',
        'insurance_client_id',
        'Incurance Clients'
    )
    ...
    def name_get(self):
        ...
        if self._context.get('category_display') == 'short':
            return super(InsuranceClientCategory, self).name_get()
