...
class AnnotationCategory(models.Model):
    _name = 'myo.annotation.category'

    name = fields.Char('Category', required=True, translate=True)
    parent_id = fields.Many2one('myo.annotation.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('myo.annotation.category', 'parent_id', 'Child Categories')
    ...
    def name_get(self):
        ...
        if self._context.get('category_display') == 'short':
            return super(AnnotationCategory, self).name_get()
