...
class Animal(models.Model):
    ...
    code = fields.Char(string='Code', required=False, default=False,
                       help='Use "/" to get an automatic new Animal Code.')

    @api.model
    def create(self, values):
        if 'code' not in values or ('code' in values and values['code'] == '/'):
            code_seq = self.pool.get('ir.sequence').next_by_code(self._cr, self._uid, 'myo.animal.code')
            values['code'] = format_code(code_seq)
        return super(Animal, self).create(values)
    ...
    def copy(self, default=None):
        ...
        default.update({'code': '/', })
        return super(Animal, self).copy(default)
