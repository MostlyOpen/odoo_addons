...
class Summary(models.Model):
    ...
    code = fields.Char(string='Summary Code', required=False, default=False,
                       help='Use "/" to get an automatic new Summary Code.')

    @api.model
    def create(self, values):
        if 'code' not in values or ('code' in values and values['code'] == '/'):
            code_seq = self.pool.get('ir.sequence').next_by_code(self._cr, self._uid, 'myo.summary.code')
            values['code'] = format_code(code_seq)
        return super(Summary, self).create(values)
    ...
    def copy(self, default=None):
        ...
        default.update({'code': '/', })
        return super(Summary, self).copy(default)
