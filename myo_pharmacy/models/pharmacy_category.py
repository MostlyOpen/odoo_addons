...
class PharmacyCategory(models.Model):
    _name = 'myo.pharmacy.category'

    name = fields.Char('Category', required=True, translate=True)
    parent_id = fields.Many2one('myo.pharmacy.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('myo.pharmacy.category', 'parent_id', 'Child Categories')
    ...
    _constraints = [(models.Model._check_recursion, 'Error! You can not create recursive categories.', ['parent_id'])]
    ...
    def name_get(self):
        ...
        if self._context.get('category_display') == 'short':
            return super(PharmacyCategory, self).name_get()
