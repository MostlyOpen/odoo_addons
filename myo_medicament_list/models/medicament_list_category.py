...
class MedicamentListCategory(models.Model):
    _name = 'myo.medicament.list.category'

    name = fields.Char('Category', required=True, translate=True)
    parent_id = fields.Many2one('myo.medicament.list.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('myo.medicament.list.category', 'parent_id', 'Child Categories')
    ...
    def name_get(self):
        ...
        if self._context.get('category_display') == 'short':
            return super(MedicamentListCategory, self).name_get()
...
class MedicamentListCategory_2(models.Model):
    _inherit = 'myo.medicament.list.category'

    medicament_list_ids = fields.Many2many(
        'myo.medicament.list',
        'myo_medicament_list_category_rel',
        'category_id',
        'medicament_list_id',
        'Medicament Lists'
    )
