...
class ProfessionalCategory(models.Model):
    _name = 'myo.professional.category'

    name = fields.Char('Category', required=True, translate=True)
    parent_id = fields.Many2one('myo.professional.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('myo.professional.category', 'parent_id', 'Child Categories')
    ...
    def name_get(self):
        ...
        if self._context.get('category_display') == 'short':
            return super(ProfessionalCategory, self).name_get()
