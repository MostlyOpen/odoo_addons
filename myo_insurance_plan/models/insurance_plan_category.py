...
class InsurancePlanCategory(models.Model):
    _name = 'myo.insurance.plan.category'

    name = fields.Char('Category', required=True, translate=True)
    parent_id = fields.Many2one('myo.insurance.plan.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('myo.insurance.plan.category', 'parent_id', 'Child Categories')
    ...
    insurance_plan_ids = fields.Many2many(
        'myo.insurance.plan',
        'myo_insurance_plan_category_rel',
        'category_id',
        'insurance_plan_id',
        'Incurance Plans'
    )
    ...
    def name_get(self):
        ...
        if self._context.get('category_display') == 'short':
            return super(InsurancePlanCategory, self).name_get()
