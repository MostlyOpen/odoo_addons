...
class Community(models.Model):
    ...
    def create(self, values):
        if 'code' not in values or ('code' in values and values['code'] == '/'):
            code_seq = self.pool.get('ir.sequence').next_by_code(self._cr, self._uid, 'myo.community.code')
            values['code'] = format_code(code_seq)
        return super(Community, self).create(values)
    ...
    def copy(self, default=None):
        ...
        default.update({'code': '/', })
        return super(Community, self).copy(default)
