...
class MedicamentCategory(models.Model):
    _name = 'myo.medicament.category'

    name = fields.Char('Category', required=True, translate=True)
    parent_id = fields.Many2one('myo.medicament.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('myo.medicament.category', 'parent_id', 'Child Categories')
    ...
    def name_get(self):
        ...
        if self._context.get('category_display') == 'short':
            return super(MedicamentCategory, self).name_get()
...
class MedicamentCategory_2(models.Model):
    _inherit = 'myo.medicament.category'

    medicament_ids = fields.Many2many(
        'myo.medicament',
        'myo_medicament_category_rel',
        'category_id',
        'medicament_id',
        'Medicaments'
    )
...
class Medicament(models.Model):
    _inherit = 'myo.medicament'

    category_ids = fields.Many2many(
        'myo.medicament.category',
        'myo_medicament_category_rel',
        'medicament_id',
        'category_id',
        'Categories'
