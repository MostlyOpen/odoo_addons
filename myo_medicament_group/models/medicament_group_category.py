...
class MedicamentGroupCategory(models.Model):
    _name = 'myo.medicament.group.category'

    name = fields.Char('Category', required=True, translate=True)
    parent_id = fields.Many2one('myo.medicament.group.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('myo.medicament.group.category', 'parent_id', 'Child Categories')
    ...
    def name_get(self):
        ...
        if self._context.get('category_display') == 'short':
            return super(MedicamentGroupCategory, self).name_get()
...
class MedicamentGroupCategory_2(models.Model):
    _inherit = 'myo.medicament.group.category'

    medicament_group_ids = fields.Many2many(
        'myo.medicament.group',
        'myo_medicament_group_category_rel',
        'category_id',
        'medicament_group_id',
        'Medicament Groups'
    )
