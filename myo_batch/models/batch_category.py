...
class BatchCategory(models.Model):
    _name = 'myo.batch.category'

    name = fields.Char('Category', required=True, translate=True)
    parent_id = fields.Many2one('myo.batch.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('myo.batch.category', 'parent_id', 'Child Categories')
    ...
    def name_get(self):
        ...
        if self._context.get('category_display') == 'short':
            return super(BatchCategory, self).name_get()
