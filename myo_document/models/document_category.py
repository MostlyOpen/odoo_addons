...
class DocumentCategory(models.Model):
    _name = 'myo.document.category'

    name = fields.Char('Category', required=True, translate=True)
    parent_id = fields.Many2one('myo.document.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('myo.document.category', 'parent_id', 'Child Categories')
    ...
    def name_get(self):
        ...
        if self._context.get('category_display') == 'short':
            return super(DocumentCategory, self).name_get()
